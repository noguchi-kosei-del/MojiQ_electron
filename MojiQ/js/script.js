 ...
 * 校正モードを切り替え
 */
function toggleProofreadingMode() {
...
function initWindowControlsAndMenuBar() {
    ...
    // --- ヘッダー用 作品仕様/校正チェックボタン ---
    const headerGdriveJsonBtn = document.getElementById('headerGdriveJsonBtn');
    const headerCalibrationBtn = document.getElementById('headerCalibrationBtn');
    const originalGdriveBtn = document.getElementById('gdriveJsonBrowserBtn');
    const originalCalibrationBtn = document.getElementById('calibrationToggleBtn');

    // ヘッダーの作品仕様ボタン → 元のボタンのクリックをトリガー
    if (headerGdriveJsonBtn && originalGdriveBtn) {
        headerGdriveJsonBtn.addEventListener('click', () => {
            originalGdriveBtn.click();
        });
    }

    // ヘッダーの校正チェックボタン → 元のボタンのクリックをトリガー
    if (headerCalibrationBtn && originalCalibrationBtn) {
        headerCalibrationBtn.addEventListener('click', () => {
