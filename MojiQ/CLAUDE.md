...
### バグ修正: 同じ文言のコメントで済スタンプが配置されない問題
- 校正チェックモードのコメントタブで、同じ文言のコメントが複数ある場合に2つ目以降のチェックボックスにチェックを入れても済スタンプが正しい位置に配置されない問題を修正
- 原因: テキスト内容が一致する最初のオブジェクトでbreakしていたため、同じ文言の2つ目以降のコメントも同じ座標を使用
- 修正内容:
  - テキスト内容が一致するオブジェクトを全て収集
  - 複数件ある場合はコメントの座標（canvasRect）に最も近いオブジェクトを選択
...
### 新機能: 校正チェック項目の確認済みチェックボックス
- 正誤・提案・コメントタブの各項目にチェックボックスを追加
- チェック時の動作:
  - 項目が薄くなる（色変更で表現）
  - 「確認済み」テキストを青色で表示
  - コメントタブ: キャンバス上のコメントテキスト左上に済スタンプを表示
- 新規データ読み込み時に確認済み状態をリセット
- ダークモード対応

### 新機能: 校正チェックモードに済スタンプ・ルビスタンプボタン追加
- カラーセクションとチェックセクションの間に「済」「ルビ」スタンプボタンを配置
- 指示入れモードの写植指示スタンプと同じ機能を校正チェックモードでも使用可能
- ボタンスタイルは指示入れモードと統一（ダークモード対応）
- モード変更イベント（mojiq:mode-changed）を追加してボタン状態を同期

### バグ修正: 校正チェックJSON読み込み時のコメント数リセット
- コメントがある状態で校正チェックJSONを読み込むとコメントタブの件数が0になる問題を修正
- 確認済み状態のみリセットし、コメント内容・件数は維持するよう変更
