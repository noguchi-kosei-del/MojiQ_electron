...
### 8. 校正モード
- **モード切替トグル**: ヘッダーのトグルスイッチで指示入れモード/校正チェックモードを切り替え
  - 選択中のモードは明るく、非選択は暗く表示
  - モード状態はlocalStorageに保存され、次回起動時に復元
- **作品仕様/校正チェックボタン**: ヘッダーに配置、ハンバーガーメニュー展開中はグレーアウト
- **レイアウト**: PDF表示を左寄せ、右側に校正チェック結果パネルを表示
- **校正チェック表示**: JSON読み込みで正誤チェック・提案チェックを右パネルに表示
- **簡略化ツール**: カラー（赤/青/カスタム/スポイト）、線の太さのみの簡易UI
- **パネル折りたたみ**: 右パネルを24pxに折りたたみ可能
- **ページジャンプ**: チェック項目のページ番号クリックで該当ページへ移動
...
| ProofreadingPanel | `js/ui/proofreading-panel.js` | 校正モードパネル |
| CalibrationPanel | `js/ui/calibration-panel.js` | 校正チェックモーダル |
...
```javascript
{
  ...
  proofreadingMode: { enabled, jsonLoaded, currentData, currentFilePath }
}
```
...
│   ├── settings.css          # 設定
│   ├── proofreading-mode.css # 校正モード
│   ├── calibration-viewer.css # 校正チェックビューワー
...
### 2026-03-02
#### 校正モード + 閲覧モード時の表示修正
- **問題**: 校正チェックモードで閲覧モードを起動すると、ツールバー部分に余白ができてしまい画像が見切れる
- **原因**: 校正モードの右サイドバーに固定幅（`flex: 0 0 400px`）が設定されており、閲覧モードに入っても幅が残ってしまう
- **修正内容**: 校正モード + 閲覧モード時に右サイドバーとツールバーを完全に非表示にするCSSルールを追加
...
#### 線の太さUIの改善
- **数値ボックスの幅調整**: 校正モードの線の太さ数値ボックスの幅を60pxに固定（`flex: 0 0 60px`）
- **両モード間の線の太さ同期**: 指示入れモードと校正チェックモードで線の太さを共通化
  - Storeの`drawing.lineWidth`を監視して両モード間で同期
  - ツール切り替え時（ペン/マーカー/消しゴム）にも線の太さがUIに反映されるように
- **修正ファイル**:
  - `js/ui/proofreading-panel.js`: Store購読による線の太さ同期
  - `js/canvas-context.js`: 線の太さ変更時にStoreに保存
  - `js/mode-controller.js`: ツール切り替え時にStoreに線の太さを保存

### 2026-02-26
#### 校正モードのボタン状態同期修正
- **問題**: 指示入れモードでコメントテキスト/ページバーを非表示にした状態で校正モードに切り替えると、ボタンのアイコン状態が反映されない（赤+斜線が表示されない）
- **原因**: `ProofreadingPanel`が`window`オブジェクトに登録されていなかったため、`script.js`の`enterProofreadingMode()`で`window.ProofreadingPanel`のチェックが失敗し、`show()`が呼ばれていなかった
- **修正内容**:
  - `proofreading-panel.js`: IIFEの最後で`window.ProofreadingPanel = api;`を追加し、グローバルからアクセス可能に
  - `show()`関数: 即座のボタン状態更新と、50ms後の再更新（DOM更新確実化）を実装
- **修正ファイル**: `js/ui/proofreading-panel.js`

#### 校正モードの実装
- **機能概要**: PDF/JPEG画像と校正チェック結果を同一画面で確認しながら校正作業ができる専用UIモードを追加
- **モード切替**:
  - ヘッダーにモード切替ボタンを追加（ハンバーガーボタンの右隣）
  - 指示入れモード: フキダシアイコン
  - 校正モード: チェックリストアイコン
  - PDF読み込み前でもモード切替可能
- **レイアウト変更（PDF読み込み後）**:
  ...
  - ツールバー: キャンバス右側に配置
  - キャンバス: 左寄せ
  - 右サイドバー: 校正パネルを表示（幅350-500px、折りたたみ可能）
- **校正パネル機能**:
  - カラー選択: 赤・青・カスタムカラー・スポイト
  - 線の太さ: 数値入力+スライダー
  - 正誤チェック/提案チェック: JSON読み込みでカテゴリ別に表示
  - ページジャンプ: チェック項目のページ番号クリックで該当ページへ移動
  - コピー機能: チェック内容をクリップボードにコピー
- **ヘッダーボタン**:
  - 指示入れモード: 作品仕様を読み込みボタン（緑アイコン）
  - 校正モード: 校正チェックボタン（青アイコン）
  - PDF読み込み前はグレーアウト
- **校正チェックJSON読み込み**:
  - 別ウィンドウを開かず、直接校正パネルにデータを表示
- **新規ファイル**:
  - `js/ui/proofreading-panel.js`: 校正パネルのUI制御
  - `css/proofreading-mode.css`: 校正モード専用スタイル
- **修正ファイル**:
  - `index.html`: モード切替ボタン、ヘッダーボタン、校正パネルHTML追加
  - `js/script.js`: モード切替ロジック、アイコン切替
  - `js/core/store.js`: 校正モード状態（proofreadingMode.enabled, jsonLoaded, currentData）
  - `js/ui/calibration-panel.js`: JSON読み込み時の別ウィンドウ表示を削除、パネルへの直接表示
  - `js/pdf-manager.js`: pdf-loadedクラスの追加

### 2026-02-24
#### 起動時間の高速化
- **問題**: 初回起動時にアプリケーションの起動が遅い
- **原因**:
  1. `checkForUpdates()`がGoogle Drive共有フォルダに同期的にアクセスし、起動をブロック
  2. スプラッシュ画面のアニメーション用に意図的な遅延が900ms設定されていた
- **修正内容**:
  - **更新チェックの非同期化**: `checkForUpdates()`をバックグラウンド実行に変更（起動をブロックしない）
  - **ネットワークアクセスのタイムアウト追加**: `fs.existsSync()`と`fs.readdirSync()`を非同期の`fs.promises.access()`と`fs.promises.readdir()`に変更し、3秒のタイムアウトを設定
  - **意図的遅延の短縮**: 900ms → 250msに短縮（100+300+200+300ms → 100+50+100ms）
- **修正ファイル**: `electron/main.js`

#### 保存機能の堅牢化
- **変更内容**:
  - **一時ファイル名の衝突防止**: ランダムサフィックスを追加（`file.tmp.timestamp-randomsuffix`形式）
  - **動的タイムアウト**: ページ数とオブジェクト数に基づいてタイムアウトを自動計算（60秒〜600秒）
  - **終了時の保存エラー通知**: 保存失敗時にユーザーにエラーダイアログを表示
- **修正ファイル**: `electron/main.js`, `js/pdf-manager.js`, `js/script.js`

#### メモリリーク対策
- **変更内容**:
  - **プリフェッチタイマーのクリーンアップ**: ページ変更時に未完了のプリフェッチをキャンセル
  - **ImageBitmapの明示的解放**: キャッシュクリア時に`ImageBitmap.close()`を呼び出し
  - **動的キャッシュサイズ**: デバイスメモリに基づいてキャッシュサイズを自動調整（5〜30ページ）
- **修正ファイル**: `js/pdf-manager.js`

#### Undo/Redoのレースコンディション修正
- **問題**: 高速なUndo/Redo操作で描画が壊れる可能性があった
- **修正内容**:
  - 5秒のタイムアウトを追加（Promise.race使用）
  - 操作IDによるステート管理で古い操作の完了を無視
- **修正ファイル**: `js/page-manager.js`

#### デッドコード・デバッグログの削除
- **削除内容**:
  - `VIEW_ROTATION`定数（未使用）
  - 開発用`console.log`（ファイル読み込み関連）
  - コメントアウトされたコード
- **修正ファイル**: `js/constants.js`, `electron/main.js`, `js/pdf-manager.js`

#### 重複コードのリファクタリング
- **変更内容**: 9つの個別スタンプ関数（renderToruStamp, renderTorutsumeStamp等）を1つの汎用関数`renderGenericTextStamp()`に統合
- **効果**: 約150行のコード削減
- **修正ファイル**: `js/drawing-renderer.js`

#### ウィンドウサイズの調整
- **変更内容**: 起動時のウィンドウ高さを900px → 1000pxに変更
- **修正ファイル**: `electron/main.js`
