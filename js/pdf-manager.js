...
window.MojiQPdfManager = (function() {
    ...
    /**
     * 保存結果の警告を表示
     * @param {object} result - 保存結果オブジェクト
     */
    function showSaveWarnings(result) {
        if (result && result.warnings && result.warnings.length > 0) {
            const warningMessage = '保存は完了しましたが、一部に問題が発生しました:\n\n' +
                result.warnings.join('\n') +
                '\n\n大量のオブジェクトがあるページでは描画が保存されていない可能性があります。';
            MojiQModal.showAlert(warningMessage, '警告');
        }
    }
